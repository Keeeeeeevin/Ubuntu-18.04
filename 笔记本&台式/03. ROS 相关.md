...
## 3. 解决 Python3 使用 cv_bridge 问题（ROS1）

* 问题：ROS1的cv_bridge工具仅支持Python2，如果要在Python3中使用，需要用源码编译。(ImportError: dynamic module does not define module export function (PyInit_cv_bridge_boost))
...
# 安装目录设定为CATKIN_WORKSPACE/install，即~/Software/catkins/catkin_cv_bridge/install
...
source ~/Software/catkins/catkin_cv_bridge/install/setup.bash --extend
...
https://github.com/ros/geometry.git -b melodic-devel（或直接下载，目录~/Software/catkins/catkin_tf/src/geometry）
https://github.com/ros/geometry2.git -b melodic-devel
cd ..
catkin_make
注：catkin_make报错：test_transform_datatype，解决办法：注释掉~/Software/catkin/catkin_tf/src/geometry2/tf2/CMakeLists.txt文件中同test_transform_datatype相关的3行（错误原因应该是没用melodic分支的源代码）。
```
