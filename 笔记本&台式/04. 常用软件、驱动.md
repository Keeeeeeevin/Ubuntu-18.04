...
----------
1. 搜狗拼音输入法
* 参考sougou help: https://pinyin.sogou.com/linux/help.php
* sougou download: https://pinyin.sogou.com/linux/
* anzhao guanwangcaozuo,chongqihou,jinxingshezhi：Configure Current Input Method --> + --> 去掉Only Show Current Language --> 搜索并添加 sogou pinyin。设置中，Sougou Pinyin需要放在keyborad-English(US)之后，否则会出现乱码的情况。


# 5. Matlab 2019a Ubuntu
----------
...
* 安装
```html
cd /home/kevin/Software/Software/pycharm-community-2021.1.2/bin
./pycharm.sh
```
### 2. 设置解释器
...
### 2. ZED ROS Wrapper（可不安装，自己写ROS节点即可）
...
* 方法：
```html
# ROS发布图像，例如/laptop_camera/image_raw，可编写程序发布或用ROS工具包发布。

# 标定
rosrun camera_calibration cameracalibrator.py --size 11x8 --square 0.045 image:=/laptop_camera/image_raw camera:=/laptop_camera --no-service-check
```

* 可能的问题：
```html
# 问题：
ImportError: dynamic module does not define init function (initcv_bridge_boost)
# 解决：https://blog.csdn.net/xiangxianghehe/article/details/79430072
修改系统环境变量配置文件~/.bashrc，暂时注释掉关于Anaconda、cv_bridge的环境变量。
