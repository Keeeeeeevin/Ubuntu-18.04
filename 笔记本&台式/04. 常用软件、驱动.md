...
* 特别注意：
```html
运行"rosrun camera_calibration camera_calibration.py"的Terminal，其CvBridge应该是原Python2.7版的，而不是为使Python3能够使用而从源码编译的CvBridge（Terminal中去掉相关的Source），否则标定界面可能会出现黑屏问题，尝试解决黑屏问题后，标定结果也是明显错误的。
```
