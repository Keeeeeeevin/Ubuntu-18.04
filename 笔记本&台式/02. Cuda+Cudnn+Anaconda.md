...
备注：（已下载，cudnn-10.2-linux-x64-v8.1.0.77.tgz) https://developer.nvidia.com/rdp/cudnn-archive
...
# 注意：过程中，Do you wish the installer to initialize Anaconda3 by running conda init? [yes|no]，选择 [no] >>> yes，否则需要按照以下说明手动设置~/.bashrc.
# 重启Terminal生效。
```
...
### 7.2. 最新版本的gym和universe不兼容，如果需要universe则按照下面版本安装
1  激活环境
```html
conda activate tf_py38
```
